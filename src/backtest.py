...
def event_backtest(
    ...
    slip=0.0005,
    max_pos=0.5,
    min_hold_bars=12,          # biraz daha uzun tut
    cooldown_bars=8,           # yeniden giriş için bekle
    rebalance_threshold=0.5,   # kolay kolay re-balance etme
    exit_confirm_bars=2,       # -1 sinyali en az 2 bar üst üste gelsin
):
    ...
    for i in range(1, len(df)):
        ...
        # ÇIKIŞ (yalnızca koşullar ve onay barı sağlanırsa)
            ...
            if (i - last_trade_i) >= cooldown_bars:
                if target <= 0:
                    ...
                    if abs(desired - target) >= rebalance_threshold * max_pos:
                        new_target = desired
            # cooldown sırasında hiçbir şey yapma

        # Hedef değiştiyse tek seferde uygula
